  ...
  useEffect(() => {
    ...
    const click = event => {
      ...
      for (let index = 1; index < points.current.length; index += 1) {
        ...
        labels.current.push(L.tooltip({ permanent: true, direction: 'center', className: 'measure-tooltip' }).setLatLng(midpoint).setContent(formatDistance(distance)).addTo(map))
      }
    }
    map.on('click', click)
