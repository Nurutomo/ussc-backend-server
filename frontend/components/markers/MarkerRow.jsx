    ...
    <tr className="animation fade-in" style={{ animationDelay: `${index * 70}ms` }}>
      ...
      <td>
        <div className="d-flex gap-1 align-items-center">
          ...
          {data.photo && (
            <MDBBtn className="p-2" tag="button" color="danger" size="sm" aria-label="Hapus foto biasa" title="Hapus foto biasa" onClick={() => onUpdate(data.id, { photo: '' })}>
          ...
          {data.photo_360 && (
            <MDBBtn className="p-2" tag="button" color="danger" size="sm" aria-label="Hapus foto 360" title="Hapus foto 360" onClick={() => onUpdate(data.id, { photo_360: '' })}>
              <i className="fas fa-trash" />
            </MDBBtn>
          )}
          <MDBBtn className="p-2" tag="label" color="light" size="sm">
      ...
      <td>
        ...
        <MDBBtn className="p-2" size="sm" color="danger" aria-label="Hapus marker dan semua foto" title="Hapus marker dan semua foto" onClick={() => onDelete(data.id)}>
          <i className="fas fa-trash" />
        </MDBBtn>
      </td>
    </tr>
